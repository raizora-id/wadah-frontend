...
import { TooltipChart } from "../base/tooltip"
...
const getMarkerBgColor = (
  ...
  if (marker === 0) {
    for (let index = 0; index < values.length; index++) {
      if (Number(values[index]) > 0) {
        return getColorClassName(colors[index] || "blue", "bg")
      }
    }
  }
  ...
  let prefixSum = 0
  for (let index = 0; index < values.length; index++) {
    prefixSum += Number(values[index])
    if (prefixSum >= marker) {
      return getColorClassName(colors[index] || "blue", "bg")
    }
  }

  return getColorClassName(colors[values.length - 1] || "blue", "bg")
}
      ...
      >
        ...
        <div className="relative flex h-2 w-full items-center">
          ...
          {marker !== undefined ? (
            ...
            >
              {marker.tooltip ? (
                <TooltipChart triggerAsChild content={marker.tooltip}>
                  <div
                    aria-hidden="true"
                    className={cn(
