...
export const CustomTooltip = ({ payload, active }: TooltipProps) => {
  ...
  const calculatePercentageDiff = () => {
    ...
    const firstValue = payload[1]?.value || 0
    const secondValue = payload[0]?.value || 0
...
export const CustomTooltip4 = ({ payload, active }: TooltipProps) => {
  ...
  const calculateDiff = () => {
    const difference = Number(payload[0]?.value) - PEER_AVERAGE
      ...
      <ul role="list" className="grid grid-cols-2 gap-x-4 p-2">
        <li className="flex space-x-2.5">
          ...
          <div className="space-y-0.5">
            ...
            <p className="font-medium text-gray-900 dark:text-gray-50">
              {formatters.percentage({ number: Number(payload[0]?.value) })}
            </p>
          </div>
        </li>
        <li className="flex space-x-2.5">
