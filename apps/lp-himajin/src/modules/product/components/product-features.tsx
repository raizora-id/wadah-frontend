...
function FeatureCard({ feature, className, zIndexOffset = 0, ...props }: FeatureCardProps) {
    ...
    const content = (
        <>
            <img src={imageUrl} alt='' className='absolute inset-0 -z-1 h-full w-full object-cover' />
            <div className='z-10 flex h-full w-full flex-col gap-2 bg-gradient-to-t from-zinc-800/40 from-15% to-transparent p-3'>
                <small className='bg-opacity-50 inline w-fit rounded-xl bg-orange-950 px-2 py-1 text-xs leading-none font-medium text-white'>
                    {category}
                </small>

                <div className='flex-1' />
                <h5 className='bg-opacity-30 rounded-xl bg-blue-950 p-3 text-base text-sm leading-none font-bold text-white backdrop-blur-sm'>
                    {title}
                </h5>
            </div>
        </>
    ...
    const containerClassName = cn(
        'relative flex h-56 w-36 flex-col overflow-hidden rounded-2xl shadow-none transition-shadow duration-300 ease-in-out hover:shadow-xl',
        className
    );
        ...
        <section className='storybook-fix flex w-full flex-col items-center gap-4 bg-orange-50 py-10'>
            <motion.header
                ...
                <Balancer className='block text-lg text-neutral-500'>
                    Quirky ceramics for happy spaces. From sleek vases to funky mugs, we&apos;ve got your shelves
                    covered.
                </Balancer>
            </motion.header>
            ...
            <motion.div
                ...
                <button
                    className='box-border inline-block h-11 transform-gpu cursor-pointer touch-manipulation rounded-full border-b-4 border-solid border-transparent bg-orange-600 px-4 py-3 text-center text-sm leading-5 font-bold tracking-wider whitespace-nowrap text-white uppercase shadow-2xl transition-all duration-200 outline-none hover:brightness-110 active:border-t-4 active:border-b-0 active:bg-none disabled:cursor-auto'
                    role='button'>
                    Ready to clay &rarr;
            ...
            <div className='relative flex w-full flex-wrap justify-center gap-4 px-2 py-12 sm:flex-row sm:gap-0'>
                <FeatureCard
                    feature={{
                        category: 'Vases',
                        imageUrl:
