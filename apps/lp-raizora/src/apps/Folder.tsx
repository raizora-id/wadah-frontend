...
export interface Config {
    ...
    name: string;
    apps: App[];
    columns?: (keyof App)[] | (Record<keyof App, string> & { mobile?: boolean })[];
    replace?: boolean;
    list?: boolean;
}
...
export default function FolderWindow(folder: Config) {
    ...
    function start(app: App) {
        if (
            app.type == 'folder' &&
            (app.replace !== undefined ? app.replace : folder.replace !== undefined ? folder.replace : true)
        )
            return replace(app);
    ...
    return (
        <div className='flex h-full w-full flex-col items-center'>
            <AnimatePresence>
                {(path.length > 1 || future.length || mobile) && (
                    <motion.div
                        ...
                        className='bg-background flex w-full origin-top items-center justify-center p-2'>
                        <div className='flex h-full w-full items-center justify-between'>
                            <div className='flex items-center space-x-1'>
                                {path.map((folder, index) => (
                                    <Fragment key={folder.id}>
                                        {index !== 0 && <p className='text-text/30 cursor-default'>/</p>}
                                        <button
                                            onClick={() => {
                                                setPath(path.slice(0, index + 1));
                                            }}
                                            className='text-text/70 hover:bg-text/10 rounded-lg px-2 py-1 transition-all'>
                                            {folder.name}
                                        </button>
                                    </Fragment>
                                ))}
                            ...
                            <div className='flex items-center space-x-1'>
                                <button onClick={back} className={`btn-icon ${path.length > 1 ? '' : 'opacity-50'}`}>
                                    <ArrowLeftIcon />
                                </button>
            ...
            {current.list ? (
                <div className={`h-full w-full max-w-full overflow-auto pb-1 ${!current.columns ? 'pt-1' : ''}`}>
                    <table className='w-full border-0'>
                        {current.columns && (
                            <thead className='bg-background h-12 border-0 select-none'>
                                <tr>
                                    <th className='bg-background sticky top-0 z-[2] border-0'></th>
                                    {current.columns.map((col, index) => (
                                        <th
                                            key={index}
                                            className={`bg-background text-text/50 sticky top-0 border-0 text-left text-sm font-medium ${
                                                typeof col === 'object' && col.mobile == false && mobile ? 'hidden' : ''
                                            }`}>
                        ...
                        <tbody>
                            {current.apps.map((app: App) => (
                                <tr
                                    key={app.id}
                                    className='hover:bg-text/5 h-12 min-w-full cursor-pointer select-none'
                                    onClick={() => start(app)}>
                                    <td className='w-0 pr-3 pl-4'>
...
export function Seo(config: Config & { level: number; parent?: App }) {
    return <SeoRenderer parent={config} apps={config.apps} level={config.level + 1} />;
}
...
export function Icon({ id }: Config) {
    ...
    return <img src={`/apps/folder/${open ? 'open' : 'closed'}.svg`} alt='Folder Icon' draggable={false} />;
}
