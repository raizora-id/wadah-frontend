...
export default function MarkdownWindow(config: Config) {
    ...
    return (
        <div className='flex h-full w-full flex-col items-center justify-start overflow-y-auto p-12'>
            <article className={`prose prose-sm sm:prose-base ${theme == 'DARK' ? 'prose-invert' : ''}`}>
                <Markdown
                    components={{
                        a: (props) => {
                                    ...
                                    onClick={() =>
                                        !props.href
                                            ...
                                            : props.href?.startsWith('#')
                                              ...
                                              : props.href?.endsWith('!')
                                                ? window.open(props.href.slice(0, props.href.length - 1), '_blank')
                                                : open({
                                                      id: props.href,
                                                      type: 'link',
...
export function Seo(config: Config & { level: number }) {
    const Heading = (level: number) => (props: any) => {
        const As = `h${Math.min(level + config.level, 6)}` as 'h1' | 'h2' | 'h3' | 'h4' | 'h5' | 'h6';
    ...
    return (
        <Markdown
            components={{
                a: (props) => <a {...props} href={props.href?.replace('#external', '')} />,
                h1: Heading(1),
                h2: Heading(2),
                h3: Heading(3),
                h4: Heading(4),
