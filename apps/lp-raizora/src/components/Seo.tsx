    ...
    return (
        <Wrapper
            ...
            {apps?.map((app) => {
                ...
                return (
                    <Fragment key={app.id}>
                        {!['link', 'markdown', 'image'].includes(app.type) && level !== 0 && (
                            <Heading>{app.name}</Heading>
                        )}
                        {props.parent && 'columns' in props.parent && props.parent.columns && (
                            <p>
                                {props.parent.columns.map((col) => {
                                    if (!props.parent) return null;

                                    const key = (typeof col == 'string' ? col : Object.keys(col)[0]) as keyof App;

                                    const label = typeof col == 'string' ? col : (Object.values(col)[0] as string);
