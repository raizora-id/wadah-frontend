        ...
        <>
            {maximized && (
                <div
                    ...
                    className='fixed inset-x-0 bottom-0 h-8'></div>
            )}
            <motion.div
                initial='hidden'
                variants={{
                ...
                animate={(!maximized || hover) && windows.length > 1 ? 'shown' : 'hidden'}
                style={Layer.style('taskbar')}
                onPointerOver={() => setHover(true)}
                onPointerLeave={() => setHover(false)}
                className='shadow-shadow/20 fixed bottom-6 left-1/2 overflow-hidden rounded-2xl bg-black/10 shadow-md backdrop-blur-md'>
                <div className='bg-text/10 flex h-full w-full items-center p-2'>
                    <AnimatePresence>
