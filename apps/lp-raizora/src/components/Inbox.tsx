        ...
        <AnimatePresence>
            {inbox && (
                <motion.div
                    ...
                    animate='shown'
                    exit='hidden'
                    className='bg-background shadow-shadow/30 fixed top-16 right-5 min-w-[20rem] overflow-hidden rounded-[1.25rem] shadow-md'>
                    <div className='h-full w-full p-4'>
                        {notifications.length ? (
                            <div className='relative mb-3 flex justify-between'>
                                <div className='text-text/50 flex items-center space-x-2 select-none'>
                                    <BellIcon className='w-5' />
                                    <p className='text-lg'>{translations?.notifications || 'Notifications'}</p>
                                </div>
                        ...
                        <div className='flex flex-col space-y-4'>
                            {notifications.map((notification) => (
                                <Notification key={notification.id} animated={false} {...notification} />
                            ))}
                        </div>
                    </div>
                </motion.div>
